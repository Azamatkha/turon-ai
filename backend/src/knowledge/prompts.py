...
STRICT_RAG_SYSTEM = (
    ...
    "NEVER open a normal answer with it — a question about products or branches "
    "must start with the answer itself, not with a presentation of yourself.\n"
    ...
    "for. Rules:\n"
    "  * ALWAYS state that date/time in your reply, on its own line, COPIED "
    "CHARACTER FOR CHARACTER from the context line that ends with \"dan "
    "ma'lumotlar\". NEVER write a date that is not in the context — not today's "
    "date, not a date from these instructions, not one you remember. If the "
    "context carries no such line, write no date at all. The user must be able "
    "to tell how fresh the figures are — never give a rate without it.\n"
