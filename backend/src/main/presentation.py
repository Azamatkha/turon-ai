...
from src.admin_panel import routers as admin_panel_routers
from src.ask import routers as ask_routers
from src.chats import routers as chats_routers
from src.simple_auth import routers as simple_auth_routers
...
def include_routers(app: FastAPI) -> None:
    ...
    # Turon AI: oddiy auth (frontend uchun) + savol-javob (RAG). Prefiks /api/v1.
    turon_router = APIRouter()
    turon_router.include_router(
        simple_auth_routers.router, prefix="/auth", tags=["Auth (simple)"]
    )
    turon_router.include_router(ask_routers.router, prefix="/ask", tags=["Ask"])
    turon_router.include_router(
        admin_panel_routers.router, prefix="/admin", tags=["Admin panel"]
    )
    turon_router.include_router(chats_routers.router, prefix="/chats", tags=["Chats"])
    app.include_router(turon_router, prefix="/api/v1")
