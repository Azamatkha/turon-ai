...
class ApplicationUnitOfWork(SQLAlchemyUnitOfWork[R]):
    ...
    def chat_sessions(self) -> ChatSessionRepository:
        """Get the ChatSessionRepository."""
        return self._get_repository(ChatSessionRepository)

    @property
    def chat_messages(self) -> ChatMessageRepository:
        """Get the ChatMessageRepository."""
        return self._get_repository(ChatMessageRepository)
