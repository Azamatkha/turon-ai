...
class DashboardStatsUseCase:
    ...
    async def execute(self) -> DashboardStatsView:
        async with self.uow as uow:
            ...
            # So'nggi faollik — oxirgi loginlar
            events = await uow.login_events.recent_with_users(uow.session, limit=8)
            recent_activity = [
                RecentActivityItem(name=name, when=when) for name, when in events
            ]
