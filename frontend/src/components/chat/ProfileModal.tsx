...
export default function ProfileModal({
  ...
  const modalStyle = isDark ? { background: "#152741", color: "#E2E8F0", border: "1px solid rgba(255,255,255,.1)" } : {};
  const inputStyle = isDark ? { background: "#1D3355", color: "#E2E8F0", borderColor: "rgba(255,255,255,.16)" } : {};
    ...
    <div onClick={onClose} className={styles.overlay}>
      <div onClick={(e) => e.stopPropagation()} className={styles.modal} style={modalStyle}>
        <div className={styles.head}>
          ...
          <HButton onClick={onClose} data-tip={s.close} aria-label={s.close} className={styles.closeBtn} baseStyle={{}} hoverStyle={{ background: "#E2E8F0", color: "#1E3A5F" }}>
        ...
        <HButton onClick={onSave} className={`${styles.saveBtn} ${saved ? styles.saveBtnSaved : styles.saveBtnIdle}`} baseStyle={{}} hoverStyle={{ transform: "translateY(-2px)", boxShadow: "0 10px 24px rgba(30, 58, 95,.28)" }}>
          {saved ? (
